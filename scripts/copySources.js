...
const aliases = Object.entries(tsconfig.compilerOptions.paths).reduce(
  (acc, [alias, values]) => {
    acc[alias.replaceAll('/', '//')] = path.resolve(
      basePath,
      values[0].replace('*', '') // Remove wildcard
    );
    return acc;
  },
  {}
...
allFiles.forEach(file => {
  ...
  if (file.endsWith('.ts') || file.endsWith('.tsx')) {
    Object.entries(aliases).forEach(([alias, value]) => {
      const regex = new RegExp(`(['"])${alias}(.*?)['"]`, 'g'); // capture potential submodule

      content = content.replace(regex, (match, quote, submodule) => {
        let relativePath = path.relative(
          path.dirname(file),
          path.resolve(basePath, value, submodule)
        );
